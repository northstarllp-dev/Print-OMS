...
import { usePortalOrderRealtime } from "@/app/portal/hooks/usePortalOrderRealtime";
...
export function OrderDetailClient({ customer, order: initialOrder, siteVisitItems = [], token }: OrderDetailClientProps) {
  ...
  const handleOrderPatch = useCallback(
    (_orderUuid: string, patch: (prev: Order) => Order) => {
      setOrder((prev) => patch(prev));
    },
    []
  );
  ...
  usePortalOrderRealtime({
    orderUuid: order.id,
    orderFriendlyId: order.orderId || order.id,
